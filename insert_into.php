...
<header id="home">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <div class="title">
             <h1>welcome in the insert of information of course page</h1>
...
<div class="container mt-5">
      <h2>Insert the information</h2>

    <div class="form-group">
            <label for="option">option you want to insert in :</label>
            <input type="text" class="form-control" name="option" required>
    </div>
    <div class="form-group">
            <label for="info">description :</label>
            <textarea class="form-control" name="info" id="" cols="10" rows="10" required></textarea>
...
if (isset($_POST["submit"])) {
    $option = $_POST['option'];
    $des = htmlspecialchars($_POST['info']);
  
     $selectedValue1 = $_COOKIE['selectedValue1'] ;
   
     $itemQuery = "SELECT item_id, MAX(stage) AS max_stage_id, option_ FROM info_table WHERE course = ? and option_ = ?";
     $stmtItem = $conn->prepare($itemQuery );
     $stmtItem->bind_param("is", $selectedValue1 , $option);
     $stmtItem->execute();
    ...
    if ($_FILES["img"]["error"] === 4) {
       ...
       $max_stage = $row['max_stage_id']+1;
        $sql2 = "INSERT INTO info_table (option_, info, course,item_id , stage , pic) VALUES (?, ?, ?, ?,?,?)";
        $stmt2 = $conn->prepare($sql2);
      
        $stmt2->bind_param("ssiiis",$option,$des ,  $selectedValue1, $row['item_id'],$max_stage , $newImageName);
