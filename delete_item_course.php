...
<header id="home">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <div class="title">
             <h1>welcome in the delete of item into course page</h1>
...
if (isset($_POST["submit"])) {
    ...
    // Prepare the SQL statement
    $stmt = $conn->prepare("SELECT id, option_ FROM info_table WHERE course = ? AND stage = ?");
    if ($stmt === false) {
        die("Error in preparing statement: " . $conn->error);
    }
    
    // Bind parameters
    $stmt->bind_param("ii", $selectedValue1, $stage);
    $stage = 0;
    ...
    $stmt->bind_result($id, $option);
    
    // Loop through the results
    while ($stmt->fetch()) {
        ?>
        <form method="POST" action="">
            <button type="submit" name="submit2" value="<?php echo $id; ?>" class="btn btn-primary"><?php echo $option; ?></button>
        </form>
        <?php
    }
...
if (isset($_POST["submit2"])) {
    ...
    $id = $_POST["submit2"];
    // Prepare the SQL statement
    $stmt = $conn->prepare("SELECT course , item_id FROM info_table WHERE id = ?");
    if ($stmt === false) {
        die("Error in preparing statement: " . $conn->error);
    }
    ...
    // Bind result variables
    $stmt->bind_result($course , $item_id);
    
    // Fetch the result
    while ($stmt->fetch()) {
        $selectedValue1 = $course;
        $selectedValue2 = $item_id;
        echo  $selectedValue2 ;
    }
    ...
    $sql = "DELETE FROM info_table WHERE course = ? and item_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $selectedValue1 ,  $selectedValue2);
